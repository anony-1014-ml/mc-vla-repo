...
from .craft_agent import CraftWorker
import time
import random
...
class SmeltWorker(CraftWorker):
    ...
    def smelting_once(self, target: str,  recipe_info: Dict, target_num, fuels):
        ...
        # roam camera
        if self.current_gui_type == 'furnace_wo_recipe':
            self._null_action()
            self.roam_camera()
            self.forget(num=0)
        ...
        # place each item to resource_0
        def place_item_to_resource_0(slot_pos, items, items_type, target_num):
            # #
            for item, _ in items.items():
                # #
                labels = self.get_labels()
                for i in range(2): del labels["resource_"+str(i)]
                item_type = items_type[item]
                inventory_id = self.find_in_inventory(labels, item, item_type)
                self._assert(inventory_id, f"not enough {item}")
                inventory_num = labels.get(inventory_id).get('quantity')
                ...
                # place 
                self.pull_item(slot_pos, inventory_id, 'resource_0', item, target_num)

                # return the remaining items
                if inventory_num > 1:
                    self.pull_item_return(slot_pos, inventory_id, item)
        
        # place each item to resource_1
        def place_item_to_resource_1(slot_pos, fuels):
            # #
            if True:
                # #
                labels = self.get_labels()
                ...
                # place 
                fuel_num = random.choice(range(1, max(min(inventory_num, 4),1)+1))
                self.pull_item(slot_pos, inventory_id, 'resource_1', fuels, fuel_num)

                # return the remaining items
                if fuel_num < inventory_num:
                    self.pull_item_return(slot_pos, inventory_id, fuels)            
        ...
        if fuels!='coals':
            ...
            for i in range(target_num):
                # #
                code_blocks = [
                    partial( place_item_to_resource_0, slot_pos, items, items_type, 1 ),
                    partial( place_item_to_resource_1, slot_pos, fuels ),
                ]
                random.shuffle(code_blocks)
                for block in code_blocks: block()
            ...
            # #
            code_blocks = [
                partial( place_item_to_resource_0, slot_pos, items, items_type, target_num ),
                partial( place_item_to_resource_1, slot_pos, fuels ),
            ]
            random.shuffle(code_blocks)
            for block in code_blocks: block()
        ...
        if result_inventory_id_1:
            ...
            result_inventory_id_list = []
            for _ in range(9):
                result_inventory_id_2 = self.find_in_inventory(labels, 'none')
                if result_inventory_id_2:
                    result_inventory_id_list.append(result_inventory_id_2)
                    labels[result_inventory_id_2]['type'] = 'resrved'
                else:
                    break
            if len(result_inventory_id_list) > 0:
                result_inventory_id_2 = random.choice(result_inventory_id_list)
            else:
                result_inventory_id_2 = None
            self._assert(result_inventory_id_2, f"no space to place result")
            self.pull_item_result(self.crafting_slotpos, 'result_0', result_inventory_id_2, target_num, target)
            # self._assert(self.get_labels().get(result_inventory_id_2).get('type') == target, f"fail for unkown reason 33333")
