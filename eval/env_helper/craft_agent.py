...
class CraftWorker(GUIWorker):
    ...
    def pull_item(self, 
        ...
        if self.slow_act:
            self._null_action(1)
        del self.gui["cursor"]
        if slot_id: self.gui[slot_id] = raw_item_name
    ...
    def find_in_inventory(self, labels: Dict, item: str, item_type: str='item', path=None):
        ...
        for key, value in labels.items():
            ...
            if item_type == "item":
                ...
                if re.fullmatch(item, str(value)):
                    return current_path
                ...
                for i in range(len(item_list)):
                    ...
                    if re.fullmatch(item_list[i][10:], str(value)):
                        return current_path
                    elif isinstance(value, dict):
                        result = self.find_in_inventory(value, item, item_type, current_path)
                        if result is not None:
    ...
    def crafting_shaped(self, target:str, iter_num:int, recipe_info: Dict,shuffle_p=0): # 1 - shuffle_p:多少概率洗牌
        ...
        # roam camera
        # if self.current_gui_type == 'crating_table_wo_recipe':
        if self.current_gui_type == 'crating_table_wo_recipe' or self.current_gui_type == 'inventory_wo_recipe':
            self._null_action()
            self.roam_camera()
            self.forget(num=0)
