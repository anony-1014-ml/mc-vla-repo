...
import action_mapping, load_model, processor_wrapper
...
class VLLM_AGENT:
    ...
    def forward(self,observations,instructions,verbos=False,need_crafting_table=False):
        ...
        # print(f"messages : {[ ( message["role"], [ content for content in message['content'] if content['type'] != 'image_url' ] ) for message in messages ]}")
        ...
        if self.LLM_backbone in {"qwen2_vl"}:
            ...
            # print(f"outputs_decode : {outputs}")
