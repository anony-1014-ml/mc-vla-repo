...
from prismatic.models.materialize import PRISMATIC_MODEL_DIR
...
class DinoSigLIPViTBackbone(VisionBackbone):
    def __init__(self, vision_backbone_id: str, image_resize_strategy: str, default_image_size: int = 224) -> None:
        ...
        self.dino_featurizer.load_state_dict(torch.load((Path(PRISMATIC_MODEL_DIR) / f"{self.dino_timm_path_or_url}.{default_image_size}.pth").resolve()))
        ...
        self.siglip_featurizer.load_state_dict(torch.load((Path(PRISMATIC_MODEL_DIR) / f"{self.siglip_timm_path_or_url}.pth").resolve()))
