...
class CategoricalActionHead(ActionHead):
    ...
    def gumbel_sample(self, logits: torch.Tensor, deterministic: bool = False, **kwargs) -> Any:
        """
    ...
    def categorical_sample(self, logits: torch.Tensor, deterministic: bool = False, **kwargs) -> Any:
        """
        Samples an action from the categorical distribution using the torch.multinomial sampling,
        or argmax for deterministic sampling.
        ...
        if deterministic:
            ...
            elif probs.dim() == 3: # [B, T, C]
                B, T, C = shape
                probs_flat = probs.view(-1, C)  # shape: [B*T, C]
                samples = torch.multinomial(probs_flat, num_samples=1)  # shape: [B*T, 1]
                return samples.squeeze(-1).view(B, T)  # shape: [B, T]
            else:
                raise ValueError(f"Unsupported shape for probs: {shape}")
    ...
    def nucleus_sample(self, logits: torch.Tensor, deterministic: bool = False, p: float = 0.85, **kwargs) -> Any:
        ...
        :type p: float
        :param \\*\\*kwargs: Additional keyword arguments (passed to gumbel_sample if deterministic).
        :returns: A sampled action. Shape: (..., *self.output_shape[:-1])
        :rtype: torch.Tensor
        """
        if deterministic:
            # return self.gumbel_sample(logits, deterministic)
            return self.categorical_sample(logits, deterministic)
        ...
        # sampled_sorted_indexes = self.gumbel_sample(sorted_log_probs, deterministic=False)
        sampled_sorted_indexes = self.categorical_sample(sorted_log_probs, deterministic=False)
    ...
    def sample(self, logits: torch.Tensor, deterministic: bool = False, **kwargs) -> Any:
        ...
        if self.nucleus_prob is None:
            # return self.gumbel_sample(logits, deterministic, **kwargs)
            return self.categorical_sample(logits, deterministic, **kwargs)
        else:
...
def make_action_head(ac_space: gymnasium.spaces.Space, pi_out_size: int, temperature: float = 1.0, **kwargs):
    """
