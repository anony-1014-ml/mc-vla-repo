...
from prismatic.action.actions import Buttons
...
class CameraHierarchicalMapping(ActionMapping):
    ...
    def to_factored(self, ac: Dict) -> Dict:
        ...
        try:
            ...
            raise Exception("Error CameraHierarchicalMapping.to_factored.")
...
class CameraParallelMapping(ActionMapping):
    """Buttons are joint as in ButtonsJointMapping, but the camera on/off meta action is now treated
    as a parallel action within the same joint space. Instead of triggering a hierarchical decision,
    the camera head operates concurrently and selects a camera action from a joint space, independently of other actions.

    :param n_camera_bins: number of camera bins in the factored space
    """

    # Exclude camera meta action from BUTTONS_GROUPS
    BUTTONS_GROUPS = ActionMapping.BUTTONS_GROUPS.copy()
    # BUTTONS_GROUPS["camera"] = ["none", "camera"]
    BUTTONS_COMBINATIONS = list(itertools.product(*BUTTONS_GROUPS.values())) + ["inventory"]
    BUTTONS_COMBINATION_TO_IDX = {comb: i for i, comb in enumerate(BUTTONS_COMBINATIONS)}
    ...
    def _precompute_to_factored(self):
        ...
        for jnt_ac, button_comb in self.BUTTONS_IDX_TO_COMBINATION.items():
            ...
            if button_comb == "inventory":
                ...
                # for group_choice in button_comb[:-1]:  # Last one is camera
                for group_choice in button_comb:
                    if group_choice != "none":
                        new_button_ac[Buttons.ALL.index(group_choice)] = 1
    ...
    def from_factored(self, ac: Dict) -> Dict:
        ...
        choices_by_group = OrderedDict(
            # (k, self.factored_buttons_to_groups(ac["buttons"], v)) for k, v in self.BUTTONS_GROUPS.items() if k != "camera"
            (k, self.factored_buttons_to_groups(ac["buttons"], v)) for k, v in self.BUTTONS_GROUPS.items()
        )
        # # Set camera "on off" action based on whether non-null camera action was given
        # camera_is_null = np.all(ac["camera"] == self.camera_null_bin, axis=1)
        # choices_by_group["camera"] = ["none" if is_null else "camera" for is_null in camera_is_null]
...
class CameraXYParallelMapping(ActionMapping):
    """Buttons are joint as in ButtonsJointMapping, but the camera on/off meta action is now treated
    as a parallel action within the same joint space. Instead of triggering a hierarchical decision,
    the camera head operates concurrently and selects a camera action from a joint space, independently of other actions.
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # camera_x
        self.camera_x_groups = OrderedDict(camera_x=[f"camera_x{i}" for i in range(self.n_camera_bins)],)
        self.camera_x_combinations = list(itertools.product(*self.camera_x_groups.values()))
        self.camera_x_combination_to_idx = {comb: i for i, comb in enumerate(self.camera_x_combinations)}
        self.camera_x_idx_to_combination = {i: comb for i, comb in enumerate(self.camera_x_combinations)}
        self.camera_x_null_idx = self.camera_x_combination_to_idx[(f"camera_x{self.camera_null_bin}",)]
        # camera_y
        self.camera_y_groups = OrderedDict(camera_y=[f"camera_y{i}" for i in range(self.n_camera_bins)],)
        self.camera_y_combinations = list(itertools.product(*self.camera_y_groups.values()))
        self.camera_y_combination_to_idx = {comb: i for i, comb in enumerate(self.camera_y_combinations)}
        self.camera_y_idx_to_combination = {i: comb for i, comb in enumerate(self.camera_y_combinations)}
        self.camera_y_null_idx = self.camera_y_combination_to_idx[(f"camera_y{self.camera_null_bin}",)]
        #
        self._null_action = {
            "buttons": self.BUTTONS_COMBINATION_TO_IDX[tuple("none" for _ in range(len(self.BUTTONS_GROUPS)))]
        }
    ...
    def _precompute_to_factored(self):
        ...
        button_dim = self.stats_ac_space["buttons"].n
        self.BUTTON_IDX_TO_FACTORED = np.zeros((len(self.BUTTONS_IDX_TO_COMBINATION), button_dim), dtype=int)
        self.CAMERA_X_IDX_TO_FACTORED = np.zeros((len(self.camera_x_idx_to_combination),), dtype=int)
        self.CAMERA_Y_IDX_TO_FACTORED = np.zeros((len(self.camera_y_idx_to_combination),), dtype=int)
        ...
        # Pre compute camera_x
        for jnt_ac, camera_comb in self.camera_x_idx_to_combination.items():
            self.CAMERA_X_IDX_TO_FACTORED[jnt_ac] = self.camera_x_groups["camera_x"].index(camera_comb[0])

        # Pre compute camera_y
        for jnt_ac, camera_comb in self.camera_y_idx_to_combination.items():
            self.CAMERA_Y_IDX_TO_FACTORED[jnt_ac] = self.camera_y_groups["camera_y"].index(camera_comb[0])
    ...
    def from_factored(self, ac: Dict) -> Dict:
        ...
        for i in range(ac["buttons"].shape[0]):
            ...
            # Camera -- inventory is also exclusive with camera
            if key == "inventory":
                key_x = (f"camera_x{self.camera_null_bin}",)
                key_y = (f"camera_y{self.camera_null_bin}",)
            else:
                key_x = (f"camera_x{ac['camera'][i][0]}",)
                key_y = (f"camera_y{ac['camera'][i][1]}",)
            new_camera_x_ac.append(self.camera_x_combination_to_idx[key_x])
            new_camera_y_ac.append(self.camera_y_combination_to_idx[key_y])

        return dict(
            buttons=np.array(new_button_ac)[:, None],
            camera_x=np.array(new_camera_x_ac)[:, None],
            camera_y=np.array(new_camera_y_ac)[:, None],
        )

    def to_factored(self, ac: Dict) -> Dict:
        """Converts an action in the new space (ac) to the factored action space. Assumes ac has a batch dim"""
        if ac["camera_x"].ndim == 0:
            ac["camera_x"] = ac["camera_x"][None]
            ac["camera_y"] = ac["camera_y"][None]
            ac["buttons"] = ac["buttons"][None]
        try:
            assert ac["camera_x"].shape[-1] == 1
            assert ac["camera_y"].shape[-1] == 1
            assert ac["buttons"].shape[-1] == 1
        except:
            # import ray
            # ray.util.pdb.set_trace()
        ...
        new_button_ac = self.BUTTON_IDX_TO_FACTORED[np.squeeze(ac["buttons"], -1)]
        new_camera_x_ac = self.CAMERA_X_IDX_TO_FACTORED[np.squeeze(ac["camera_x"], -1)]
        new_camera_y_ac = self.CAMERA_Y_IDX_TO_FACTORED[np.squeeze(ac["camera_y"], -1)]
        new_camera_ac = np.stack((new_camera_x_ac, new_camera_y_ac),axis=-1)

        return dict(buttons=new_button_ac, camera=new_camera_ac)

    def get_action_space_update(self):
        return {
            "camera_x": spaces.MultiDiscrete([len(self.camera_x_combinations)]),
            "camera_y": spaces.MultiDiscrete([len(self.camera_y_combinations)]),
            "buttons": spaces.MultiDiscrete([len(self.BUTTONS_COMBINATIONS)])
        }
