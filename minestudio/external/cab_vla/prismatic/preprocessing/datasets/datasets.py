...
class FinetuneDataset(Dataset[Dict[str, torch.Tensor]]):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if isinstance(self.tokenizer, (GemmaTokenizer, GemmaTokenizerFast)):
            is_multimodal = "image" in self.examples[idx]
            if is_multimodal:
                # sigle turn #
                conversation = conversation[:2]
                for turn_idx, turn in enumerate(conversation):
                    # Get "effective" string added to prompt --> handle whitespace for tokenizer type!
                    msg = prompt_builder.add_turn_for_multimodal(turn["from"], turn["value"], is_multimodal=is_multimodal)
                    ...
                    if (turn_idx % 2) == 0:
                        ...
                        inputs = self.tokenizer(
                            prefix.replace(IMAGE_TOKEN, IMAGE_TOKEN * self.tokenizer.image_seq_length),
                            text_pair=suffix,
        ...
        if "image" in self.examples[idx]:
            ...
            # Process Image --> get "pixel_values" (will either be a torch.Tensor OR a Dict[str,torch.Tensor])
            if isinstance(self.image_transform, Compose) or hasattr(self.image_transform, "is_prismatic"):
                ...
                pixel_values = self.image_transform(Image.open(self.image_dir / image_path).convert("RGB"), return_tensors="pt")["pixel_values"][0]

            return dict(pixel_values=pixel_values, input_ids=input_ids, labels=labels, token_type_ids=token_type_ids)

        else:
            # No image --> return `pixel_values` = None; Collator will do the smart batch handling for us!
            return dict(pixel_values=None, input_ids=input_ids, labels=labels, token_type_ids=token_type_ids)
