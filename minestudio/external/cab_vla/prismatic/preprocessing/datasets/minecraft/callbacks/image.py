...
FilePath: /minecraft/callbacks/image.py
'''
import re
import io
import av
...
from prismatic.preprocessing.datasets.minecraft.callbacks.callback import ModalKernelCallback, ModalConvertCallback
...
class ImageKernelCallback(ModalKernelCallback):
    ...
    def do_decode(self, chunk: bytes, **kwargs) -> np.ndarray:
        ...
        with io.BytesIO(chunk) as input:
            if self.num_workers > 0:
                ...
                for packet in packet_generator:
                    for av_frame in packet.decode():
                        frame = convert_and_resize(av_frame, self.frame_width, self.frame_height)
                        frames.append(frame)
                stream.close()
                container.close()
    ...
    def do_merge(self, chunk_list: List[bytes], **kwargs) -> np.ndarray:
        ...
        if self.num_workers > 0:
            ...
            frames = [self.do_decode(chunk) for chunk in chunk_list]
