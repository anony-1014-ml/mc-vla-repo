...
FilePath: /minecraft/dataset_event.py
'''
import io
import re
import os
...
from prismatic.preprocessing.datasets.minecraft.utils import batchify
from prismatic.preprocessing.datasets.minecraft.core import KernelManager
from prismatic.preprocessing.datasets.minecraft.callbacks import ModalKernelCallback
from prismatic.preprocessing.datasets.minecraft.register import Registers
...
class EventKernel:
    ...
    def __init__(self,
        ...
        min_within: Optional[int] = None,
        max_within_list: Optional[List[Tuple[str,int]]] = None,       
        ) -> None:
        ...
        self.local.txn = self.lmdb_stream.begin(write=False)

        if ( txn := self.local.txn ) is not None:
        ...
        if min_within is not None:
            for event in sorted(list(self.event_info.keys())):
                if self.get_event_size(event) < min_within:
                    del self.event_info[event]
        #
        ...
        self.filter_out(min_nearby, max_within_list)
    ...
    def filter_out(self,
        ...
        for event in self.event_list:
            ...
            for i in range(num_events):
                ...
                #     break
                if max_within_list is not None:
                    max_within_flag = False
                    for max_within_event_regex, max_within_num in max_within_list:
                        if max_within_num < 0: continue
                        if re.match(max_within_event_regex, event):
                            if len(remaining_events[event]) >= max_within_num: max_within_flag = True
                            break
                    if max_within_flag: break
...
class EventDataset(Dataset):
    ...
    def __init__(self, 
        ...
        min_within: Optional[int] = None, # minimum number of samples within each event
        max_within_list: Optional[List[Tuple[str,int]]]=None, # maximum number of samples within each event
    ) -> Any:
        super().__init__()
        ...
        self.event_kernel = EventKernelManager(
            ...
            max_within_list=max_within_list,
        )
