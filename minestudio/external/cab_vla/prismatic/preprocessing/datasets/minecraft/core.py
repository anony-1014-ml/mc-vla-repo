...
from prismatic.preprocessing.datasets.minecraft.utils import pull_datasets_from_remote
...
class ModalKernel(object):
    ...
    def __init__(self, source_dirs: List[str], modal_kernel_callback: ModalKernelCallback, short_name: bool = False):
        ...
        for source_dir in source_dirs:
            for lmdb_path in source_dir.iterdir():
                ...
                self.stream_dict[lmdb_path] = lmdb.open(lmdb_path, max_readers=128, lock=False, readonly=True)
                self.local.txn_dict[lmdb_path] = self.stream_dict[lmdb_path].begin()
                if ( txn := self.local.txn_dict[lmdb_path] ) is not None:
                    ...
                    for chunk_info in __chunk_infos__:
                        chunk_info['lmdb_path'] = lmdb_path
                        if short_name:
    ...
    def read_chunks(self, eps: str, start: int, end: int) -> List[bytes]:
        ...
        for chunk_id in range(start, end + self.chunk_size, self.chunk_size):
            if ( txn := self.local.txn_dict[meta_info['lmdb_path']] ) is not None:            
...
if __name__ == "__main__":
    from prismatic.preprocessing.datasets.minecraft.callbacks import ImageKernelCallback, ActionKernelCallback, MetaInfoKernelCallback
