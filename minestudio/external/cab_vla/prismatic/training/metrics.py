...
class TensorboardTracker:
    ...
    def initialize(self) -> None:
        self.writer = SummaryWriter(Path( self.run_dir / f"{self.run_id}_tb"))

    @overwatch.rank_zero_only
    def write_hyperparameters(self) -> None:
        hparams_text = '\n'.join([f'{key}: {value}' for key, value in self.hparams.items()])
...
class Metrics:
    def __init__(
        ...
        self.loss_list_size = 5
        for loss_idx in range(self.loss_list_size): self.state[f"loss_{loss_idx}"] = deque(maxlen=window_size)
    ...
    def commit(
        ...
        for key, value in kwargs.items():
            if key == "loss":
                ...
                for loss_idx in range(min(self.loss_list_size, len(value))): self.state[f"loss_{loss_idx}"].append(value[ loss_idx ].detach())
            else:
    ...
    def push(self) -> str:
        ...
        # #
        loss_list = []
        for loss_idx in range(self.loss_list_size):
            loss_sub = torch.stack(list(self.state[f"loss_{loss_idx}"])).mean().item() if len(self.state[f"loss_{loss_idx}"]) > 0 else None
            loss_list.append( loss_sub )
        ...
        for loss_idx in range(self.loss_list_size):
            if loss_list[ loss_idx ] is not None: metrics[ f"{prefix}/Loss_{loss_idx}"] = loss_list[ loss_idx ]
