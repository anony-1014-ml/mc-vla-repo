...
def dict_map(fn, d):
    ...
    if isinstance(d, Dict) or isinstance(d, DictConfig):
        ...
        return {k: dict_map(fn, v) if k != "condition" else v for k, v in d.items()}
    else:
        return fn(d)
...
class MinePolicy(torch.nn.Module, ABC):
    ...
    def __init__(self, hiddim, action_space=None, temperature=1.0, nucleus_prob=None) -> None:
        ...
        if hiddim > 0:
            self.set_heads(hiddim, action_space=action_space, temperature=temperature, nucleus_prob=nucleus_prob)

    def set_heads(self, hiddim, action_space=None, temperature=1.0, nucleus_prob=None) -> None:
    ...
    def get_action(self,
        ...
        """
        if input_shape == "*":
            # print(f"image : {input['image'].shape}, condition : {input['condition']}")
    ...
    def _batchify(self, elem):
        ...
        if isinstance(elem, (int, float)):
            return torch.tensor(elem, device=self.device)
