...
class RecordCallback(MinecraftCallback):
    ...
    def __init__(self, record_path: str, fps: int = 20, frame_type: Literal['pov', 'obs'] = 'pov', recording: bool = True,
                    show_actions=False,record_actions=False,record_infos=False,record_rewards=False,record_origin_observation=False, prefix=None,
                 **kwargs):
        """
        ...
        :param record_rewards: Whether to save reward to a JSON file.
    ...
    def after_reset(self, sim, obs, info):
        ...
        if self.recording:
            ...
            if self.record_rewards:
                self.rewards.append(float(0)) # 0.0 for reset                
    ...
    def _save_episode(self):
        ...
        if len(self.frames) == 0:
            return 
        output_path = self.record_path / ( f'episode_{self.episode_id:06}.mp4' if self.prefix is None else f'{self.prefix}_episode_{self.episode_id:06}.mp4' )
        ...
        if self.record_origin_observation:
            output_origin_path = self.record_path / ( f'episode_{self.episode_id:06}.npy' if self.prefix is None else f'{self.prefix}_episode_{self.episode_id:06}.npy' )
        ...
        if self.record_actions: # assert self.actions>0 sense self.frame > 0
            output_action_path = self.record_path / ( f'episode_{self.episode_id:06}.action.json' if self.prefix is None else f'{self.prefix}_episode_{self.episode_id:06}.action.json' )
        ...
        if self.record_infos: # assert self.actions>0 sense self.frame > 0
            output_info_path = self.record_path / ( f'episode_{self.episode_id:06}.info.json' if self.prefix is None else f'{self.prefix}_episode_{self.episode_id:06}.info.json' )
            # output_info_path = self.record_path / ( f'episode_{self.episode_id:06}.info.json.gz' if self.prefix is None else f'{self.prefix}_episode_{self.episode_id:06}.info.json.gz' )
            record_infos = [self._process_info(info) for info in self.infos]
            if Path(output_info_path).suffix == ".gz":
                with gzip.open(output_info_path, "wt", encoding="utf-8") as file:
        ...
        if self.record_rewards: # assert self.actions>0 sense self.frame > 0
            output_reward_path = self.record_path / ( f'episode_{self.episode_id:06}.reward.json' if self.prefix is None else f'{self.prefix}_episode_{self.episode_id:06}.reward.json' )
            record_rewards = [self._process_reward(reward) for reward in self.rewards]
            with open(output_reward_path, 'w', encoding="utf-8") as file:
                json.dump(record_rewards, file)
            self.rewards = []
    ...
    def _process_info(self,info:dict):
        ...
        if "pov" in record_info and self.frame_type == 'pov':
            del record_info['pov']
        if "image" in record_info:
            del record_info["image"]
    ...
    def _process_reward(self,reward:float):
        """
        Processes the reward dictionary for saving.

        Converts numpy arrays to lists.

        :param reward: The reward dictionary.
        :return: The processed reward dictionary.
        """
        record_reward = deepcopy(reward)
        record_reward = self._convert_data(record_reward)
