...
from minestudio.models import VPTPolicy, SteveOnePolicy
...
class PlayCallback(MinecraftCallback):
    ...
    def __init__(
        ...
        if agent_generator is not None:
            if isinstance(agent_generator, partial): print(f'[green]Load agent with name: {agent_generator.func.__name__}, args: {agent_generator.keywords}[/green]')
    ...
    def before_step(self, sim, action: Any) -> Dict:
        ...
        if isinstance(action, str) or action is None:
            ...
            if self.switch != "agent":
                ...
                if   isinstance(self.agent, VPTPolicy):
                    agent_action, self.memory = self.agent.get_action(
                        sim.obs,
                        self.memory,
                        input_shape = "*"
                        )
                elif isinstance(self.agent, SteveOnePolicy):
                    agent_action, self.memory = self.agent.get_action(
                        input={'image' : sim.obs['image'], 'condition' : { 'cond_scale': 4.0, 'text' : self.condition_text } },
                        state_in=self.memory,
                        input_shape='*'
                        )
        ...
        if self.gui.chat_message is not None: #!WARNING: should stop the game when chat message is not None 
            self.condition_text = self.gui.chat_message
